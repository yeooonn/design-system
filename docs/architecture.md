...
- `@yeoooonn/ds-tokens` 값을 React Native `ViewStyle` / `TextStyle`로 매핑
- 컴포넌트별 `*Styles.ts`에 variant/size/state resolver 분리
- RN `style` prop으로 적용 — NativeWind/Tailwind **미사용**
- 소비 앱에 Tailwind/NativeWind 설정을 강제하지 않음 → **web과 동일한 배포·의존성 철학**

**패턴**

```
components/Button/
  index.tsx          # Pressable + a11y + 이벤트
  buttonStyles.ts    # resolveButtonStyles, size/variant 타입
  Button.stories.tsx # on-device Storybook
```

- **Compound Component**: Modal, Card, BottomSheet (`Card.Header` 등)
- **Provider + Hook**: ThemeProvider, ToastProvider + useToast
- **앱 전용**: BottomSheet (Reanimated + PanResponder), KeyboardAvoid

**웹과의 차이**

| 항목 | 웹 | 앱 |
| --- | --- | --- |
| 스타일 | inline style + resolver (`CSSProperties`) | inline style + resolver (`ViewStyle`) |
| 커스터마이즈 | `style`, `className` | `style` |
| 이벤트 | `onClick` | `onPress` |
| Icon | SVG URL + CSS mask | children slot |
| Select | native `<select>` / custom | BottomSheet 옵션 목록 |
| BottomSheet | — | Reanimated + PanResponder (앱 전용) |

초기 설계는 NativeWind v4를 검토했으나, npm DS 목표와 web 패리티·소비 앱 설정 부담을 고려해 **inline style + resolver**로 통일했습니다. (과거 ADR: [ds-app NativeWind 포트 설계](../superpowers/specs/2026-07-13-ds-app-nativewind-design.md))
...
## 스타일링: Inline Style vs Tailwind

현재 `@yeoooonn/ds-web`과 `@yeoooonn/ds-app` 모두 **inline style + token resolver**를 사용합니다.

| 기준 | Inline (현재) | Tailwind / NativeWind |
| --- | --- | --- |
| 소비 앱 의존성 | react (+ RN peer) | Tailwind/NativeWind 빌드 파이프라인 필요 |
| web·app 스타일 공유 | tokens + resolver 패턴 | className 문법으로 유사 표현 가능 |
| variant 로직 | TypeScript resolver로 명시적 | CVA + className |
| 번들 | 런타임 CSS-in-JS / css-interop 없음 | purged CSS 또는 className 변환 레이어 |
| Storybook | 추가 CSS 프레임워크 설정 없음 | `@tailwind` + postcss / metro 설정 |

**권장**: npm 라이브러리로 배포하는 현재 목표라면 inline 유지가 유리합니다. 소비 앱이 Tailwind를 쓰더라도 DS가 Tailwind를 강제하지 않습니다.

Tailwind/NativeWind 전환을 검토할 시점:

- web·app 스타일 표현을 className 수준에서 더 맞추고 싶을 때
