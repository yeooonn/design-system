  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button label={`${colorScheme === 'light' ? '🌙 다크' : '☀️ 라이트'} 모드`} onClick={toggleTheme} variant="secondary" />
      </div>

      {sizes.map((size) => (
        <section key={size} style={{ marginTop: 32 }}>
          <h2 style={{ fontSize: 14, color: theme.text.secondary, textTransform: 'uppercase' }}>size: {size}</h2>
